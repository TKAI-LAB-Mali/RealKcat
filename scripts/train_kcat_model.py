...
# Define directories
model_code = 'outputs'
result_filename_csv = f"{model_code}/kcat_results.xlsx"
...
train_path = f'{save_dir}/train_dataset_y1_wPafA.pt'
val_path = f'{save_dir}/val_dataset_y1_wPafA.pt'
test_path = f'{save_dir}/test_dataset_y1.pt'
...
model_path = "outputs/kcat_model.pkl"  # Define model save path here
print("\nTraining the kcat model...")
kcat_model = train_model(params, train_data.cpu().numpy(), train_labels.cpu().numpy(), val_data.cpu().numpy(), val_labels.cpu().numpy(), model_path=model_path, seed=seed)

# Step 5: Make Predictions and Evaluate Model
print("\nEvaluating the model...")
train_pred = kcat_model.predict(train_data.cpu().numpy())
val_pred = kcat_model.predict(val_data.cpu().numpy())
test_pred = kcat_model.predict(test_data.cpu().numpy())
...
os.makedirs("outputs", exist_ok=True)
confusion_matrix_path = "outputs/confusion_matrix_kcat.png"
tsne_plot_path = "outputs/tsne_plot_kcat.png"
