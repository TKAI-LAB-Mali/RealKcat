...
class KcatInference:
    ...
    def load_test_data(self):
        ...
        self.y1_test_WT_tensor = self.y1_test_WT_tensor.to(self.device)

    def standardize_test_data(self, global_mean_1, global_std_1, global_mean_2, global_std_2):
        wt_dataset = TensorDataset(self.X_test_WT_tensor, self.y1_test_WT_tensor)
        md_dataset = TensorDataset(self.X_test_MD_Ala_tensor, self.y1_test_WT_tensor)
    ...
    def convert_to_numpy(self):
        X_CatAware_WT_test_data, test_CatAware_WT_y1 = dataset_to_tensors(self.testCatAware_WT_dataset_std)
        X_CatAware_MD_test_data, test_CatAware_MD_y1 = dataset_to_tensors(self.testCatAware_MD_dataset_std)
        return X_CatAware_WT_test_data.cpu().numpy(), test_CatAware_WT_y1.cpu().numpy(), X_CatAware_MD_test_data.cpu().numpy(), test_CatAware_MD_y1.cpu().numpy()
    ...
    def compare_predictions(self, y_true_wt, y_pred_wt, y_pred_md):
        comparison_df = pd.DataFrame({
            'True y1 (WT)': y_true_wt,
            'Predicted y1 (WT)': y_pred_wt,
            'Predicted y1 (MD)': y_pred_md
        })
        comparison_df['MD_lower_than_WT'] = comparison_df['Predicted y1 (MD)'] < comparison_df['Predicted y1 (WT)']
        comparison_df['MD_lower_than_WT_andTruePred'] = (comparison_df['MD_lower_than_WT'] &
                                                         (comparison_df['True y1 (WT)'] == comparison_df['Predicted y1 (WT)']))
    ...
    def plot_comparisons(self, y_true, y_pred, y_md_pred):
        ...
        plt.title('True vs Predicted y1 (WT)')
        plt.legend()
        plt.grid(True)
        plt.savefig(f'{self.output_dir}/WT_vs_Predictions_test_ood_kcat.png')
        plt.close()
        ...
        plt.xlabel('Predicted y1 (WT)')
        plt.ylabel('Predicted y1 (MD)')
        plt.title('MD vs WT Predictions')
        plt.legend()
        plt.grid(True)
        plt.savefig(f'{self.output_dir}/MD_vs_WT_Predictions_test_ood_kcat.png')
        plt.close()
...
if __name__ == "__main__":
    ...
    inference = KcatInference(model_path=model_path, test_file_path=test_file_path, device=device)
    ...
    inference.display_prediction_ranges(y_pred_WT, class_ranges_y1)